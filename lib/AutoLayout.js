...
export function AutoLayout(props) {
    ...
    useEffect(() => {
        try {
            if (typeof props.url !== 'undefined') {
                ...
                try {
                    const root = props.content.meta.layout.json['layout:root:column:0']
    ...
    if (typeof props.content === 'undefined' && typeof props.url === 'undefined') {
        return <h6 style={{ color: 'red' }}>Please pass props.content or props.url</h6>
    }

    return (
        <div id='pvl'>
            {pvl?.children ? (
                <Choose layout={pvl.children} components={props.components} />
            ) : error ? (
                error
            ) : (
                'loading'
